...
class WC_Comment_Images {
	...
	public function add_metaboxes() {
		...
		$metaboxes->add_field(
			array(
				...
				'name'     => esc_html__( 'Upload review images ', 'woodmart' ),
				'default'  => $ids,
				'priority' => 10,
			)
		);
	...
	public function get_image_ids_array( $comment_id = 0 ) {
		$image_ids_from_meta = $this->get_image_ids_meta( $comment_id );

		if ( empty( $image_ids_from_meta ) ) {
			$ids = array();
		} else {
			$ids = explode( ',', $image_ids_from_meta );
		}
	...
	public function display_images( $comment_content ) {
		if ( ! $this->has_images() || ! woodmart_get_opt( 'single_product_comment_images', '1' ) || ( ! wp_doing_ajax() && ( is_admin() || ! is_singular( 'product' ) ) ) ) {
