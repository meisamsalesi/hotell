...
class WOODCORE_Import extends WP_Importer {
	...
	function import( $file, $version = false ) {
		...
		// XTemos.
		$previously_imported_data = get_option( 'wd_imported_data_' . $version );
		$data                     = $this->wd_data;

		if ( $previously_imported_data ) {
			$data = array_replace_recursive( $data, $previously_imported_data );
		}

		update_option( 'wd_imported_data_' . $version, $data );
		// XTemos.
	}
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( $term_id ) {
				...
				// XTemos.
				$this->wd_data['term']['category'][ $term_id ] = array(
					'new'  => $term_id,
					'old'  => $term_id,
					'type' => 'category',
				);
			...
			// XTemos.
			$this->wd_data['term']['category'][ $id ] = array(
				'new'  => $id,
				'old'  => $cat['term_id'],
				'type' => 'category',
			);
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( $term_id ) {
				...
				// XTemos.
				$this->wd_data['term'][ $term['term_taxonomy'] ][ $term_id ] = array(
					'new'  => $term_id,
					'old'  => $term_id,
					'type' => $term['term_taxonomy'],
				);
				// XTemos.
				continue;
			}
			...
			// XTemos.
			$this->wd_data['term'][ $term['term_taxonomy'] ][ $term['term_id'] ] = array(
				'new'  => $id['term_id'],
				'old'  => $term['term_id'],
				'type' => $term['term_taxonomy'],
			);
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			// XTemos.
			$this->wd_data['all_posts'][ $post['post_id'] ] = array(
				'old'       => $post['post_id'],
				'new'       => $post_id,
				'post_type' => $post['post_type'],
			);

			$this->wd_data[ $post['post_type'] ][ $post['post_id'] ] = array(
				'old'       => $post['post_id'],
				'new'       => $post_id,
				'post_type' => $post['post_type'],
			);
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( '_elementor_data' === $meta['key'] ) {
						$this->wd_data['all_posts'][ $post['post_id'] ]['type']          = 'elementor';
						$this->wd_data[ $post['post_type'] ][ $post['post_id'] ]['type'] = 'elementor';
					}
					// XTemos.
				}
			}
	...
	function process_menu_item( $item ) {
		...
		if ( 'taxonomy' === $_menu_item_type ) {
			if ( isset( $this->wd_data['term'][ $_menu_item_object ][ $_menu_item_object_id ]['new'] ) ) {
				$_menu_item_object_id                                     = $this->wd_data['term'][ $_menu_item_object ][ $_menu_item_object_id ]['new'];
				$menu_item_meta['_menu_item_object_id']                   = $_menu_item_object_id;
				$this->processed_terms[ intval( $_menu_item_object_id ) ] = intval( $_menu_item_object_id );

			}
		} elseif ( 'post_type' === $_menu_item_type ) {
			if ( isset( $this->wd_data['all_posts'][ $_menu_item_object_id ]['new'] ) ) {
				$_menu_item_object_id                                     = $this->wd_data['all_posts'][ $_menu_item_object_id ]['new'];
				$menu_item_meta['_menu_item_object_id']                   = $_menu_item_object_id;
				$this->processed_posts[ intval( $_menu_item_object_id ) ] = intval( $_menu_item_object_id );
			}
		}
		// XTemos.
		...
		if ( isset( $_menu_item_menu_item_parent ) && isset( $this->processed_menu_items[ intval( $_menu_item_menu_item_parent ) ] ) ) {
			$_menu_item_menu_item_parent = $this->processed_menu_items[ intval( $_menu_item_menu_item_parent ) ];
		} elseif ( isset( $_menu_item_menu_item_parent ) && $_menu_item_menu_item_parent ) {
		...
		$args = array(
			...
			'menu-item-object'      => isset( $_menu_item_object ) ? $_menu_item_object : '',
			'menu-item-parent-id'   => isset( $_menu_item_menu_item_parent ) ? $_menu_item_menu_item_parent : '',
			'menu-item-position'    => intval( $item['menu_order'] ),
			'menu-item-type'        => isset( $_menu_item_type ) ? $_menu_item_type : '',
			...
			'menu-item-classes'     => isset( $_menu_item_classes ) ? $_menu_item_classes : '',
		...
		if ( $id && ! is_wp_error( $id ) ) {
			// XTemos.
			$this->wd_data['all_posts'][ $item['post_id'] ]     = array(
				'old'       => $item['post_id'],
				'new'       => $id,
				'post_type' => 'nav_menu_item',
			);
			$this->wd_data['nav_menu_item'][ $item['post_id'] ] = array(
				'old'       => $item['post_id'],
				'new'       => $id,
				'post_type' => 'nav_menu_item',
			);
	...
	function fetch_remote_file( $url, $post ) {
		...
		for ( $i = 1; $i <= apply_filters( 'woodmart_safe_remote_iterations', 3 ); $i++ ) {
			$remote_response = wp_safe_remote_get(
				$url,
				array(
					'timeout'  => apply_filters( 'woodmart_safe_remote_timeout', 5 ),
					'stream'   => true,
	 ...
	 */
	function parse( $file ) {
		$parser = new WOODCORE_WXR_Parser();
